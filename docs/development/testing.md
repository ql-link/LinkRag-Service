...
- parse_result 消费兜底测试：失败分类与当前任务过滤用 Mockito 单测（`KnowledgeParseResultServiceImplTest`、`KnowledgeParseStuckScannerTest`、`ParseResultKafkaConfigTest`）；“坏消息不阻塞后续 / 缓存补偿隔离”用 `@EmbeddedKafka` 集成测试（`ParseResultConsumerEmbeddedKafkaTest`，按 `*Test` 命名以纳入 Surefire）。所有用例须断言 Java 不写业务表（`verify(...never()).updateById`）。
