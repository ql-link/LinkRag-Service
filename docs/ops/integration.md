...
- Python 读取 Markdown 配套图片：`GET /api/v1/internal/files/{fileId}/assets?path={normalizedRelativePath}`。该接口仅暴露 Java 已归一化并上传的相对图片路径，配置服务 token 时 normalized Markdown URL 会追加 `token` 查询参数，接口也接受 `Authorization: Bearer <token>`；部署上仍依赖内网隔离。
...
- **Markdown 配套图片归一化**：用户上传 Markdown 时可同时上传图片目录内文件，前端按 `assets[]` + `assetRelativePaths[]` 传入。Java 端扫描 Markdown 本地图片引用（含 inline、尖括号路径、HTML img 与 reference-style 图片），按归一化相对路径匹配配套图片，上传原始 Markdown、normalized Markdown、图片对象和 manifest；`document_original_file.object_key` 指向 normalized Markdown，Python 读取 `/content` 时拿到的就是可解析文档。配套图片只允许常见图片格式并校验内容；Markdown 中未匹配到的图片保留原链接并写入 manifest；用户点击解析时 Java 返回 `frontendStatus=asset_missing`、`missingAssets` 和 `canContinue=true` 给前端提醒，前端可带 `ignoreMissingAssets=true` 继续解析，不强制补齐全部图片。
