...
> 头像与反馈附件不新增独立桶配置，复用 `MINIO_PUBLIC_BUCKET` 公开桶。头像对象 key 格式为 `avatar/{userId}/{uuid}.{suffix}`，公开访问地址优先按 `tolink.oss.public-base-url` 生成并写入 `sys_user.avatar_url`；反馈附件对象 key 由 Java 生成，格式为 `feedback/yyyy/MM/{uuid}.{suffix}`（精度到月），数据库只存 object key，可访问 URL 由后端按公开桶拼装。
