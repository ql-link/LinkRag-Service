...
| `DOCUMENT_FILE_SERVICE_TOKEN` | 内部原文件与 Markdown 配套图片读取 Token；图片 URL 在配置后会追加 `token` 查询参数，接口也接受 `Authorization: Bearer <token>` | 否 | 空 | `your-service-token-here` |

Markdown 配套图片内部 URL 依赖 `DOCUMENT_FILE_INTERNAL_BASE_URL` 拼装。配置 `DOCUMENT_FILE_SERVICE_TOKEN` 后，normalized Markdown 中的图片 URL 会携带内部 `token` 查询参数；部署时仍必须通过网络策略确保仅 Python RAG 服务可访问，不能暴露到公网。
