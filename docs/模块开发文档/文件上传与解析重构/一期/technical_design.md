...
| `docs/组件和数据库约定/middleware_contract.md` | MySQL、Redis、MQ、OSS、API 与日志追踪公共约定 | 确认本期新增 Redis key、移除 MySQL 配置表依赖、复用 OSS 路径边界，并明确公共契约需要回写 |
| `docs/组件和数据库约定/middleware-components/oss_component.md` | OSS 接入方式 | 业务层使用组件，不改 framework |
| `docs/组件和数据库约定/middleware-components/redis_component.md` | Redis 接入方式 | 新增业务缓存服务，不把配置逻辑写入 framework |
| `docs/组件和数据库约定/middleware-components/kafka_component.md` | MQ 接入方式 | 一期不投递解析 MQ，不接入 Kafka；二期解析 MQ 投递阶段再按该组件文档设计 |
...
- `knowledge:file-upload:config` 是新增 Redis 公共 key，需要回写 `docs/组件和数据库约定/middleware_contract.md` Redis 章节。
