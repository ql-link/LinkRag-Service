...
| 框架 | 版本 | 用途 |
|------|------|------|
| MyBatis-Plus | 3.5.2 / 3.4.2 | ORM 框架，减少手写 SQL，内置分页与条件构造支持 |
...
**删除策略**：
- 当前对话、数据集、知识文件以物理删除为主
- 由应用层显式处理级联删除和对象存储清理
- 不依赖 MyBatis-Plus 逻辑删除字段
...
| user_id | VARCHAR(36) | NOT NULL | 所属用户 ID |
| dataset_id | VARCHAR(36) | NOT NULL | 所属数据集 ID |
...
**索引说明**：
- `idx_user_active_list`：复合索引，包含 (user_id, is_pinned, updated_at)，覆盖用户对话列表查询
...
│   │   ├── ChatConversation.java        # 对话（绑定 datasetId）
