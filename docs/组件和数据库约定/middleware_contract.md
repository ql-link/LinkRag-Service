...
- 状态字段因业务不同而不同，如 `status`、`is_active`、`is_default`、`upload_status`、`task_status`

### 4.4 知识文件表职责

- `document_original_file` 只记录原文件上传事实、归属关系、上传状态、失败原因编码和原文件对象定位。
- `document_parsed_file` 只记录某个原文件对应的一条解析业务记录，与 `document_original_file` 保持一对一；允许上传成功后先存在未解析记录，`parse_count` 初始为 0，`latest_parse_task_id` 初始为空。
...
| `user:role:{userId}` | 用户角色快速失效 | `DoubleDeleteCacheService` 预留驱逐能力 |
| `llm:cfg:{configId}` | 用户 LLM 配置缓存 | LLM 配置修改后的双删目标 |
| `llm:u_def:{userId}` | 用户默认配置缓存 | 默认配置变更后的双删目标 |
| `llm:pvd:{providerType}` | 系统厂商信息缓存 | 厂商变更后的双删目标 |
...
- 当前 Redis 组件已提供 `DoubleDeleteCacheService`
- 当前双删对象包括：
  - LLM 配置缓存：`llm:cfg:{configId}`
  - 用户默认配置缓存：`llm:u_def:{userId}`
  - 系统厂商缓存：`llm:pvd:{providerType}`
  - 用户信息缓存：`user:info:{userId}`
  - 用户角色缓存：`user:role:{userId}`
- 当前双删策略为：
  - 第一删：同步删除，最多重试 3 次，重试间隔 200ms
  - 第二删：延迟 1 秒再次删除
...
```text
original/user-{userId}/dataset-{datasetId}/{yyyy}/{MM}/{dd}/{fileId}/{originalFilename}
```

- 当前知识解析产物对象 key 生成规则约定为：

```text
parsed/user-{userId}/dataset-{datasetId}/{yyyy}/{MM}/{dd}/{taskId}/{parsedFilename}
```
...
| `original/user-{userId}/dataset-{datasetId}/{yyyy}/{MM}/{dd}/{fileId}/{originalFilename}` | 知识原始文件对象 key | 用于原始文件归档与隔离 |
| `parsed/user-{userId}/dataset-{datasetId}/{yyyy}/{MM}/{dd}/{taskId}/{parsedFilename}` | 知识解析产物对象 key | 用于解析产物归档与隔离，bucket 为 `rag-md` |
