...
本文件只写公共约定，不写某次功能的具体技术方案，也不展开组件内部完整实现。

## 2. 使用规则

- AI 在需求分析后的技术设计阶段必须读取本文件
- 若本次开发新增或修改 MySQL 字段、Redis key、MQ topic、OSS path、API 契约，应在方案与落地后同步更新本文件
- 若只是使用现有约定而不新增公共契约，可只在 `technical_design.md` 中引用，不强制更新本文件
...
| `tolink.rag.parse_result` | 回传解析结果 | 由解析侧发送，Java 服务侧消费 |
| `tolink-java-parse-result-worker` | 解析结果消费组 | Kafka 默认消费者组 |

### 6.4 消息结构

- 消息体必须具备可追踪业务主键
- 当前已落地的消息体采用 envelope + payload 结构
- 当前 envelope 字段：
  - `mq_type`
  - `mq_name`
  - `payload`
- 当前解析结果消息 `parse_result` 的 payload 字段包括：
  ...
  - `failure_reason`
  - `time_cost_ms`
- 当前解析任务消息 `parse_task` 的 payload 字段包括：
  - `task_id`
  - `document_id`
  - `file_url`
  - `file_type`
