...
| --- | --- | --- |
| `tolink.rag.parse_task` | 投递原始知识文件解析任务 | 由 Java 创建解析任务后发送给 Python 解析侧 |
| `tolink.rag.parse_result` | 回传解析结果 | 历史结果回传 topic；二期主链路改为 Python 写库并回调 Java，不再作为 Java 写库主链路 |
...
- 当前解析结果消息 `parse_result` 的 payload 字段包括：
  ...
  - `failure_reason`
  - `time_cost_ms`
- 当前二期解析任务消息 `parse_task` 改为扁平 JSON，字段包括：
  - `task_id`
  - `original_file_id`
  - `user_id`
  - `dataset_id`
...
- 当前代码现状：
  ...
  - 解析结果消费通过 `KnowledgeParseResultKafkaReceiver` 进入 `KnowledgeParseResultService`
