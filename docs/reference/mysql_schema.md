...
| `knowledge_file_config` | `KnowledgeFileConfig` | Deprecated knowledge file upload config history; Redis is the runtime source |
...
- `knowledge_file_config` is retained for rollback and historical compatibility only. Current knowledge file upload config is resolved from Redis key `knowledge:file-upload:config`, with `tolink.knowledge-file.*` as the default fallback.
