...
- `llm_user_config.capability`（Entity `UserLLMConfig.capability`，单数）为专用能力标识，`VARCHAR(32) NOT NULL DEFAULT 'CHAT'`，取值 `CHAT` / `EMBEDDING` / `RERANK` / `OCR`；列名以单数 `capability` 为准（曾误用复数 `capabilities`，已对齐线上库）。
