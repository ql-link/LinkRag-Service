...
> **项目名称**：ToLink Service
> **模块名称**：文件上传与解析协同重构（一期）
> **需求文档**：`docs/module-development-files/storage-file-management/一期/requirement.md`
> **分支名称**：skill-test
> **技术负责人：** AI 协作草拟
> **最后更新时间：** 2026-04-25
...
| `docs/architecture/middleware-components/oss_component.md` | OSS 组件职责和接入方式 | 业务层只使用 OSS 服务，不改 OSS framework |
| `docs/architecture/middleware_contract.md` | MySQL、OSS 公共约定 | 表命名、公共字段、对象存储定位按项目契约收敛 |
