...
Java 不再提供召回 session 换票或召回中转接口。登录/注册响应中的 `accessToken` 是 RS256 JWT，
同时作为 Java Sa-Token 登录凭证；前端使用同一枚 token 以 `Authorization: Bearer` 直连 Python
`/api/v1/rag/stream`。Python 使用 Java 公钥独立验签，Java 不在召回/生成请求路径上。
