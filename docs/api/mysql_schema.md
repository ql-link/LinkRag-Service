...
- 默认厂商与模型种子采用“全量保留 + 部分启用”：`scripts/db/seed_llm_providers.sql` 由本地 Docker MySQL 当前 `llm_system_provider` / `llm_provider_model` 快照生成，保留 144 个厂商与 4941 条模型能力记录；默认只启用常见厂商 `linkrag` / `openai` / `claude` / `gemini` / `deepseek` / `aliyun` / `glm` / `moonshot` / `xai` / `minimax` / `baichuan` / `baidu` / `hunyuan` / `jina` / `mimo` / `volcengine` / `xunfei`，其余厂商默认下架。模型层同样全量保留，但只启用每个常见厂商少量近期主推模型（本次生成 79 条启用模型能力行），避免用户侧与管理端翻阅列表过长。`model_name` 保持真实调用 ID，`display_name` 使用短展示名：保留主版本号（如 `2.5` / `4.8`），避免把发布日期、快照号或冗长版本标识暴露给前端。`scripts/db/schema.sql` 不再维护独立轻量 LLM 目录，初始化 LLM 目录必须执行 `seed_llm_providers.sql`。
