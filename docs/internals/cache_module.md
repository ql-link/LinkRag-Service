...
- 缓存按「索引 + 按厂商分片」组织，避免单一大 key（旧版 `llm:pvd:catalog` 全量快照约 150KB）：
  - `llm:pvd:catalog:index`：启用厂商索引 `ProviderCatalogIndex`（轻量引用 `ProviderRef` 列表，含 id/type/name/iconUrl/priority，priority 倒序），不含模型，约数 KB。`iconUrl` 来自 `llm_system_provider.icon_url`，用于用户侧厂商目录展示图标。
  - `llm:pvd:catalog:models:{providerType}`：单厂商模型分片 `ProviderModelShard`（该厂商全部上架模型，全 capability 未过滤），每个约数 KB。分片值 `models` 为 `List<ProviderModel>`，随 `ProviderModel` 实体新增 `protocol` / `api_base_url`（模型能力层事实字段，见 `docs/api/mysql_schema.md`「协议与入口三层语义」）和 `display_name`（模型短展示名）自动携带，无需改缓存代码——这些字段会随分片一起命中缓存并组装进 `GET /api/v1/llm/providers` 响应。
...
- 发布刷新（协议/展示名改造发布期一次性动作）：`ProviderModel` 新增 `protocol` / `api_base_url` / `display_name` 后，上线前缓存中的旧序列化分片不含这些字段，反序列化会得到 `null protocol` 或空展示名，可能被 `setup-provider` 读到并污染用户配置快照或展示效果。发布步骤须刷新（删除）`llm:pvd:catalog:*` 键（索引 `llm:pvd:catalog:index` + 各厂商分片 `llm:pvd:catalog:models:*`），强制下一次读回源重建带新字段的分片；或为 `MODELS_KEY_PREFIX` 加版本号使旧 key 自然失效。日常字段写入无此问题，写路径 evict 已覆盖。
...
- 接口 `GET /api/v1/llm/configs` 经 `UserLLMConfigServiceImpl` 返回用户可用配置：用户自配配置由 `UserLLMConfigCacheService` 提供 read-through 缓存，LinkRag 只读配置直接查 `llm_system_preset` 后合并到响应。`GET /api/v1/llm/configs/default` 改由 `EffectiveLLMConfigServiceImpl` 解析，先查用户自配默认，未命中再查 LinkRag 系统兜底预设。
...
- 读取：缓存未命中时按 `user_id` 一次性回源 `llm_user_config` 的用户自配配置并转 DTO；随后合并 LinkRag 当前系统默认预设为 `isEditable=false` 的只读配置项。`providerType` / `capability` / `isActive` 过滤，以及用户自配默认配置筛选（`capability + is_default=true + is_active=true + is_system_preset=false`）均在内存完成，避免不同筛选条件拆出多份缓存。系统兜底预设不进入用户配置缓存。
