...
## Java / Python LLM 配置缓存协定

- Java 用户 LLM 配置写库成功后，先通过 `CacheConsistencyService` 清理 Java 侧 `LLM_CONFIG` 与 `USER_DEFAULT_LLM_CONFIG` 缓存。
- 同一事务提交后，Java 通过 `RagCacheSyncNotifier` 发送 `tolink.rag.cache_sync`。
- 新增、更新、设置默认发送 `action=refresh`；删除发送 `action=invalidate` 并携带 `config_id`。
- Python RAG 端消费后，`refresh` 会清理该用户所有配置缓存并清理 ModelFactory 客户端缓存；`invalidate` 会清理指定配置缓存并清理该用户客户端缓存。
- 系统预设配置使用 `llm_user_config.user_id=0`。Java 发送 `user_id=0 + action=refresh` 时，Python 应清理全量 LLM 配置缓存、系统 provider 缓存和全部 ModelFactory client 缓存。
- 当前 Java 发送链路不引入 LLM 专用 outbox 表，发送失败记录日志并依赖 MQ/补偿体系后续收敛；如果生产要求强一致，需要另行引入可靠投递或 CDC 补偿。
