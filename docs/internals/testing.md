...
- 对话轮次落库测试（chat_turn）：信封解包（先取 `payload` 再反序列化、snake_case 映射、兼容扁平）、`status` 枚举与必填校验用 `ChatTurnMQTest`；单事务落库（success/partial/failed 三类写 `chat_message`+`llm_usage_log`+更新 `chat_conversation` 与首轮标题）、`request_id` 幂等去重跳过、`conversation_id` 归属不匹配/会话缺失丢弃、标题唯一约束冲突保留默认用 `ChatTurnPersistenceServiceImplTest`（Mockito 单测，`@BeforeAll` 初始化 `ChatMessage`/`ChatConversation`/`UsageLog` 的 MP 列缓存）。`chat_message` 改一行一轮后，`ChatMessageTest` 断言新字段（`query`/`answer`/`references` JSON+`JacksonTypeHandler`/`request_id`/`status`）并断言旧字段（`role`/`content`/`token_count`）已移除；H2 测试库 `references` 降级 VARCHAR 存 JSON 文本。
