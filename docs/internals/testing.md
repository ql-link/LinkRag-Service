...
- 对话轮次落库测试（chat_turn）：信封解包（先取 `payload` 再反序列化、snake_case 映射、兼容扁平）、`status` 枚举与必填校验用 `ChatTurnMQTest`；单事务落库（success/partial/failed 三类写 `chat_message`+`llm_usage_log`+更新 `chat_conversation` 与首轮临时标题）、`request_id` 幂等去重跳过、`conversation_id` 归属不匹配/会话缺失丢弃、标题生成异步触发用 `ChatTurnPersistenceServiceImplTest`（Mockito 单测，`@BeforeAll` 初始化 `ChatMessage`/`ChatConversation`/`UsageLog` 的 MP 列缓存）。模型标题生成成功回写、失败保留临时标题、用户手动标题不覆盖、默认 CHAT 配置回退用 `ConversationTitleServiceImplTest`。`chat_message` 改一行一轮后，`ChatMessageTest` 断言新字段（`query`/`answer`/`references` JSON+`JacksonTypeHandler`/`request_id`/`status`）并断言旧字段（`role`/`content`/`token_count`）已移除；H2 测试库 `references` 降级 VARCHAR 存 JSON 文本。对话消息读取接口由 `ChatControllerTest` 真实 H2 集成覆盖，需断言同一会话下按 `created_at` 正序分页返回 `query`/`answer`/`configId`/`modelName`/`references`/`requestId`/`status`。历史召回片段恢复的批量 Chunk 详情接口由 `KnowledgeChunkControllerTest` 覆盖，需断言按请求 chunk id 顺序返回、过滤越权/REMOVED/不存在片段、回填 `document_original_file.original_filename`、未登录 401。
