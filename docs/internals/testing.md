...
- 对话轮次落库测试（chat_turn，后台续跑 + 可靠落库 chat-stream-resilient-persist）：信封解包（先取 `payload` 再反序列化、snake_case 映射、兼容扁平）、`status` 三态（`GENERATING`/`COMPLETED`/`FAILED`）枚举、`turn_id`/`request_id` 必填、`GENERATING` 起点 `provider_type` 空、`FAILED` 携带 `error_code`/`error_message` 用 `ChatTurnMQTest`；按 `turn_id` upsert（`GENERATING` 起点插行不入账、终态更新同行补 `error_*`、`GENERATING`→`COMPLETED` 推进同一行且用量只入一次、终态后迟到 `GENERATING` 不回退、重复终态/重复 `GENERATING` 幂等跳过）、`conversation_id` 归属不匹配/会话缺失丢弃、终态成功才异步触发标题用 `ChatTurnPersistenceServiceImplTest`（Mockito 单测，`@BeforeAll` 初始化 `ChatMessage`/`ChatConversation`/`UsageLog` 的 MP 列缓存）；端到端按 `turn_id` upsert/状态不回退/错误字段落库/归属拒写用 `ChatTurnIntegrationTest`（H2 真实链路）。模型标题生成成功回写、失败保留临时标题、用户手动标题不覆盖、默认 CHAT 配置回退用 `ConversationTitleServiceImplTest`。`ChatMessageTest` 断言一行一轮字段（含 `turn_id`/`query`/`answer`/`references` JSON+`JacksonTypeHandler`/`request_id`/`status`/`error_code`/`error_message`）并断言旧字段（`role`/`content`/`token_count`）已移除；H2 测试库 `references` 降级 VARCHAR 存 JSON 文本、`turn_id` 唯一索引 `uk_chat_message_turn_id`。
- 全链路用量上报测试（usage_report，LINK-184）：信封解包与字段映射（`user_id` string→Long、可空字段缺省、vision 真实 `completion_tokens`）、`stage`/`operation` 枚举与必填 token 校验、出站序列化往返用 `UsageReportMQTest`；落库字段映射、`config_id`/`request_id`/`latency_ms` 缺省落 NULL、`status` 缺省补 `success`、token 缺省补 0 用 `UsageReportPersistenceServiceImplTest`（Mockito 单测，仅 mock `UsageLogMapper`）。`chat_turn` 落库补 `stage='chat'`/`operation='generate'` 的全链路口径一致性由既有 `ChatTurnPersistenceServiceImplTest` 落库断言间接覆盖。
