...
- 博客测试：`BlogContentStorageServiceImplTest` 覆盖 Markdown UUID Key、UTF-8/后缀校验、无业务大小限制和图片 MIME；`BlogControllerTest` 使用 `@SpringBootTest` + H2 + local OSS 覆盖管理员创建/上传/发布、公开列表不返回正文、公开详情、普通用户 403、图片资源软删和无下载路由。Sa-Token HTTP 角色校验依赖 `SaTokenAnnotationConfig`，`StpInterfaceImplTest` 必须覆盖字符串形式 loginId。
