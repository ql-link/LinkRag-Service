...
- 反馈模块测试：`FeedbackServiceImplTest` 覆盖匿名提交默认值、附件上传后保存 `attachment_object_key`、非法 `type` 拒绝、入库失败时清理刚上传的私有附件；`AdminFeedbackServiceImplTest` 覆盖管理员分页、状态终态写 `processed_at`、优先级更新、回复不自动改状态；`OssApplicationServiceImplTest` 与 `OssObjectKeyGeneratorTest` 覆盖 `feedback/yyyy/MM/dd/{uuid}.{suffix}` 私有 object key 规则。
