...
- LLM 配置重构（厂商—模型—能力三层 + LinkRag 系统兜底 + 两步配置）测试：目录按能力聚合/空厂商过滤、用户侧目录多厂商一次性批量查模型（`ProviderModelService.listActiveModelsByProviderIds` 走 `provider_id IN (...)`，`SystemProviderServiceImplTest` 以 `verify(times(1))` 守护「无论多少厂商只触发一次模型查询」即 N+1 不复发，空厂商列表短路不查库）用 `SystemProviderServiceImplTest`、`ProviderModelServiceImplTest`（含管理员新增模型即时反映、幂等上架、空 ID 列表短路）；配置厂商整厂商展开 + Key 厂商级共用 + 加密脱敏 + 模型启停 + 按能力选生效（含 `MODEL_DISABLED`/`MODEL_NOT_SUPPORTED` 拒绝）+ 清空用户自配默认 + 配置列表合并 LinkRag 只读项用 `UserLLMConfigServiceImplTest`（走 `LambdaUpdateWrapper.set`，`@BeforeAll` 调 `TableInfoHelper.initTableInfo` 初始化 `UserLLMConfig` / `SystemPreset` 的 MP 列缓存）；系统预设字段复制、同能力默认唯一、禁止直接禁用当前默认用 `SystemPresetServiceImplTest`；有效配置解析（用户自配默认优先、无用户默认时回退 `source=SYSTEM` 的 LinkRag 系统预设）用 `EffectiveLLMConfigServiceImplTest`；注册不再写预设由 `AuthServiceImplTest` 覆盖；两步接口端到端用 `ConfigControllerTest`（`@SpringBootTest`+H2，setup 先插 `llm_provider_model` 目录，再 setup-provider→effective→toggle-model→default）。
