...
- 统一 LLM 配置测试：`LLMModelConfigValidatorTest` 锁定“不存在→停用→越权→能力不匹配”优先级；`LLMCapabilityDefaultServiceImplTest` 覆盖 USER 默认覆盖、SYSTEM 回退、清除与缺省错误；`LLMModelConfigServiceImplTest` 覆盖 SYSTEM/USER 保存、加密脱敏、默认事务与引用删除保护；`ConfigControllerTest` 和 `AdminLLMConfigControllerTest` 覆盖 configId-only DTO、旧身份别名不存在、默认接口和管理端原子保存。厂商/模型目录的 N+1、协议、入口与候选审核仍由 `SystemProviderServiceImplTest`、`ProviderModelServiceImplTest`、`ProviderModelSyncServiceImplTest` 承接。
  - 管理端模型配置测试需覆盖目录复制与目录变更二选一、API Key 重新加密、SYSTEM 默认替换、当前默认删除保护、配置/默认原子回滚和脱敏输出。
  - 外部模型目录候选同步（LINK-50）测试：`ProviderModelSyncServiceImplTest` 覆盖手动刷新只写 `llm_provider_model_sync_job` / `llm_provider_model_sync_candidate`、新增/匹配/疑似过期统计、重复刷新同一 `(providerId, syncSource, modelName, inferredCapability)` 时更新候选而非重复插入、候选发布复用 `ProviderModelService.addModelCapability` 写入正式目录、候选审核状态更新；测试不得依赖真实 `models.dev` 网络返回，外部源通过 `ExternalModelCatalogClient` mock。
