...
- 单事务三写：① `INSERT chat_message`（一行一轮）；② `INSERT llm_usage_log`（关联 `conversation_id` / `message_id` / `request_id`）；③ `UPDATE chat_conversation` 的 `last_config_id` / `last_model_name` / `updated_at`，首轮由 `query` 生成临时标题。事务提交后，Java 通过 `conversationTitleExecutor` 异步调用用户本轮 Chat 配置（不可用则回退默认 CHAT 配置）的 OpenAI-compatible chat completions 生成自然短标题；失败、拒绝、配置不可用或用户已手动改成其它标题时保留当前标题。
