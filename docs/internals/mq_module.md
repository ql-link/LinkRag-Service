...
| `RagCacheSyncMQ` | `tolink.rag.cache_sync` | Java -> Python |
...
- 用户 LLM 配置变更通过 `RagCacheSyncMQ` 通知 Python 清理缓存。Java 仅在事务提交后发送；消息体只包含 `user_id`、`config_id`、`action`，不包含 API Key。
