...
- MQ trace 只走传输 header，不改 Java/Python 共享消息体：`MQSend` 适配层通过 `link-observability` 的 `TraceHeaders` 把当前 MDC 的 `trace_id` 自动写入 `X-Trace-Id`；Kafka/RabbitMQ 消费入口读取 `X-Trace-Id` / `x-trace-id` / `trace_id` / `trace-id` 并用 `TraceContext.start(...)` 恢复 MDC，缺失或非法时自建 trace，`finally` 清理。
- 当前部署 vendor 为 RabbitMQ。`RabbitMQAutoConfiguration` 统一声明 durable Queue、`<queue>.DLX` 与 `<queue>.DLT`；listener 首次投递加 3 次本地重试，耗尽后 reject 且不 requeue。可选 `x-delayed-message` 插件默认关闭，当前业务不使用延迟消息。
...
- 链路：`ChatTurnRabbitReceiver`（薄适配层 + header trace 恢复）→ `ChatTurnConsumer`（实现 `ChatTurnMQ.MQReceiver`）→ `ChatTurnPersistenceService`（`@Transactional` 单事务）；Kafka receiver 仅用于回滚。
...
- Queue/DLX/DLT 由 `RabbitMQTopologyScanner` 扫描 `ChatTurnMQ` 后统一声明；消费使用 Boot simple listener 配置。
...
- 链路：`UsageReportRabbitReceiver`（薄适配层 + header trace 恢复）→ `UsageReportConsumer`（实现 `UsageReportMQ.MQReceiver`）→ `UsageReportPersistenceService`，每条上报 `INSERT llm_usage_log` 一行。
...
- Queue/DLX/DLT 由 `RabbitMQTopologyScanner` 扫描 `UsageReportMQ` 后统一声明。
