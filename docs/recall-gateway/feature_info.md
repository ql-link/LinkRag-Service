...
| 来源 | `java-gateway-brief.md`（原文迁移）；issue `LINK-35`；关联 Python internal recall stream `.specs/recall-http-api/brief.md`；后续优化 `LINK-40`（短期 token 直连 Python SSE） |
| 分支 | feature/recall-gateway（已从 `dev` 切出，2026-05-30） |
| 当前阶段 | 已发 PR #30（→ dev，branch-pr-workflow，2026-05-30，https://github.com/ql-link/LinkRag-Service/pull/30）；质量审查通过、R1/R2 已修、全量 `mvn test` 117 绿、doc-sync/ai-links 通过；待 review/合并 |
