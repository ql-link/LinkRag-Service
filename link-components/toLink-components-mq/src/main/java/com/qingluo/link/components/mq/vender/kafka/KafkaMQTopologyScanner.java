...
/**
 * Scans business message model classes so Kafka topics can be declared from models.
 */
public class KafkaMQTopologyScanner implements ResourceLoaderAware {

    private static final Logger log = LoggerFactory.getLogger(KafkaMQTopologyScanner.class);
    ...
    private java.util.Optional<AbstractMQ> instantiateMessageModel(String className) {
        try {
            ...
            log.warn("Skip Kafka MQ model [{}], it must implement AbstractMQ and provide a no-arg constructor", className, ex);
            return java.util.Optional.empty();
        }
    }
