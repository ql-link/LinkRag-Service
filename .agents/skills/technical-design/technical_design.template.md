...
*规范：涉及前端对接时，必须按接口或响应 DTO 分组写清楚成功响应、分页响应、空响应、关键展示字段和前端判断字段，不要只放一个 happy path 示例。响应示例默认只写展示和业务判断必需字段，不要暴露服务端内部定位字段、对象存储 bucket/object key、内部 URL、Token、密钥、异常堆栈或其他敏感信息；若确需前端访问文件，应单独设计带权限校验的下载/预览接口。*
...
*规范：如果涉及多张表，必须按表拆成独立小节或独立表格书写，不要把多张表字段混在同一张字段表中。*
