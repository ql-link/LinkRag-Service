...
为减少流程噪音并保持单主流程推进，阶段切换统一采用“双重审核放行”：

1. AI 自检通过（`AI_REVIEW=PASS`）
2. 人工审核通过（`HUMAN_REVIEW=APPROVED`）

仅当以上两项同时满足时，才允许进入下一阶段。

每个阶段结束后，AI 必须输出统一状态块：

```text
PHASE: <当前阶段>
AI_REVIEW: PASS | FAIL
HUMAN_REVIEW: APPROVED | PENDING
NEXT_PHASE: <下一阶段名> | BLOCKED
BLOCK_REASON: <阻塞原因；无则写 NONE>
```

若任一条件不满足，`NEXT_PHASE` 必须为 `BLOCKED`，并等待人工处理。
...
1. 当前模块当前期次目录全部产物（`feature_info.md`、`requirement.md`、`technical_design.md`、`implementation_report.md`、`testing_delivery.md`）
2. 本次实际代码 diff
3. `.agents/skills/code-review-and-quality/SKILL.md`

AI 必须执行：

- 按 `code-review-and-quality` 做五维审查（Correctness / Readability / Architecture / Security / Performance）
- 输出分级问题（`Critical` / `Required` / `Suggestion`）
- 给出审查结论（`APPROVE` / `REQUEST_CHANGES`）
- 汇总本次模块当前期次目录产物、代码改动摘要、风险与交付建议
