...
/**
 * 链路追踪入口过滤器：为每个 HTTP 请求建立 trace_id 并写入 MDC，全链路日志可串联。
 *
 * <p>优先复用上游传入的 {@code X-Trace-Id}（便于跨服务串联，如网关/前端/Python 端透传），
 * 缺失<strong>或非法</strong>时新建——入站值经 {@link TraceContext#isValid} 白名单校验，
 * 拒绝含换行等控制字符的值，防止日志注入（CWE-117）。trace_id 同时回写响应头，方便前端与排查时定位。
 * 请求结束在 finally 清理 MDC，避免 Tomcat 线程复用导致 trace_id 串到下一个请求。</p>
 */
@Component
