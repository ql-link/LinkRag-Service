...
public class DocumentParseStuckScanner {
    ...
    public void scan() {
        ...
        List<DocumentParsedLog> candidates = documentParsedLogMapper.selectList(
            new LambdaQueryWrapper<DocumentParsedLog>()
                .eq(DocumentParsedLog::getTaskStatus, TASK_CREATED)
                .lt(DocumentParsedLog::getCreatedAt, coarseCutoff));
        if (candidates == null || candidates.isEmpty()) {
            return;
        }
        for (DocumentParsedLog candidate : candidates) {
            // 单条隔离，避免一条异常中断整批扫描。
            try {
    ...
    private void handleCandidate(DocumentParsedLog candidate, LocalDateTime now) {
        ...
        DocumentParsedLog fresh = documentParsedLogMapper.selectById(candidate.getId());
        if (fresh == null) {
            return;
        }
    ...
    private DocumentParseResultMQ.MsgPayload toPayload(DocumentParsedLog log, DocumentParseFile parseFile) {
