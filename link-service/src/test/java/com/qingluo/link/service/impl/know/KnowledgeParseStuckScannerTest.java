...
class KnowledgeParseStuckScannerTest {
    ...
    private KnowledgeParseStuckScanner newScanner(ParseResultStuckProperties properties) {
        return new KnowledgeParseStuckScanner(parsedLogMapper, parseFileMapper, sseService, properties, metrics);
    }
    ...
    void Should_RepushFromDb_When_OverThresholdAndDbTerminal() {
        KnowledgeParsedLog candidate = log(201L, "task-1", 101L, 301L, "created", minutesAgo(6));
        KnowledgeParsedLog fresh = log(201L, "task-1", 101L, 301L, "success", minutesAgo(6));
    ...
    void Should_DoNothing_When_NotOverThreshold() {
        ...
        KnowledgeParsedLog candidate = log(201L, "task-1", 101L, 301L, "created", minutesAgo(3));
        given(parsedLogMapper.selectList(any())).willReturn(List.of(candidate));
        given(parseFileMapper.selectById(301L)).willReturn(parseFile(301L, 401L, "task-1"));
    ...
    void Should_Skip_When_CandidateIsNotCurrentTask() {
        KnowledgeParsedLog candidate = log(201L, "task-old", 101L, 301L, "created", minutesAgo(6));
        given(parsedLogMapper.selectList(any())).willReturn(List.of(candidate));
    ...
    void Should_RespectPerDatasetThreshold(long datasetId, int datasetThresholdMin, int elapsedMin, boolean hit) {
        ...
        KnowledgeParsedLog candidate = log(201L, "task-1", 101L, 301L, "created", minutesAgo(elapsedMin));
        given(parsedLogMapper.selectList(any())).willReturn(List.of(candidate));
    ...
    private KnowledgeParsedLog log(Long id, String taskId, Long originalFileId, Long parseFileId,
    ...
    private KnowledgeParseFile parseFile(Long id, Long datasetId, String latestTaskId) {
