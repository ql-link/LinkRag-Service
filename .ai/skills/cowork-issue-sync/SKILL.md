...
每次创建 issue 前，调用 Linear `list_users`（team = `QIngluo`）动态获取当前团队成员列表，展示给用户并询问选择哪一位负责人。不得使用硬编码列表，不得自动猜测。
...
Linear 与 GitHub 使用固定映射，不降级到其他标签：
...
1. 先检查 Linear 是否存在目标标签；缺失则补建后再创建 issue。
2. 按映射表在 GitHub 创建对应标签；缺失则补建，不降级到 `enhancement` 等其他标签。
...
在对应仓库创建 GitHub issue，正文使用步骤 2 生成的内容，并在正文末尾追加 `## Linear` 段落写入 Linear issue URL。
...
### 步骤 5：将 GitHub 链接写入 Linear links 区域

使用 `save_issue` 的 `links` 字段，将 GitHub issue URL 以 `{url, title}` 形式添加到 Linear issue 的链接区域（title 格式：`GitHub #<number>`）。

不修改 Linear 正文，不发 comment。
...
- assignee
- 使用或新建的 labels
- 是否存在部分失败（如 Linear 正文更新失败）

## 7. 失败处理

- Linear 创建失败：终止流程，不创建 GitHub issue
- Linear 成功但 GitHub 创建失败：保留 Linear issue，并明确报告 GitHub 失败
- Linear links 写入失败：不回滚主 issue，报告”主记录已创建，但 Linear links 区域的 GitHub 链接未写入”
## 8. 质量门禁

- 项目归属必须准确，不可把 issue 发到错误仓库
- Linear links 区域必须包含 GitHub issue URL；GitHub 正文末尾必须包含 Linear issue URL
- 标题不能空泛，正文不能缺失核心背景和边界
- 负责人必须由用户从动态获取的成员列表中明确选择，不可自行指定
...
生成正文时以同目录下的示例文件为质量基准：

- `example-bug.md`：修复 Bug 模板示例
- `example-feature.md`：新增需求模板示例（Feature / Improvement 通用）
