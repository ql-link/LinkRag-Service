...
CREATE TABLE IF NOT EXISTS user_login_event (
    ...
    login_source    VARCHAR(16) NOT NULL,
    created_at      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_sys_user_created_at ON sys_user(created_at);
CREATE INDEX IF NOT EXISTS idx_user_login_event_created_user ON user_login_event(created_at, user_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_llm_model_config_owner_model ON llm_model_config(scope, owner_user_id, provider_id, model_name, capability);
CREATE INDEX IF NOT EXISTS idx_llm_model_config_owner_capability ON llm_model_config(scope, owner_user_id, capability, is_active);
CREATE UNIQUE INDEX IF NOT EXISTS uk_llm_capability_default_owner_cap ON llm_capability_default(scope, owner_user_id, capability);
CREATE INDEX IF NOT EXISTS idx_llm_capability_default_config ON llm_capability_default(config_id);
...
CREATE TABLE IF NOT EXISTS dataset_parse_config (
    ...
    enhancement_chat_config_id BIGINT,
    enhancement_vision_config_id BIGINT,
    rerank_config_id           BIGINT,
...
CREATE INDEX IF NOT EXISTS idx_dataset_parse_enhancement_chat_config ON dataset_parse_config(enhancement_chat_config_id);
CREATE INDEX IF NOT EXISTS idx_dataset_parse_enhancement_vision_config ON dataset_parse_config(enhancement_vision_config_id);
CREATE INDEX IF NOT EXISTS idx_dataset_parse_rerank_config ON dataset_parse_config(rerank_config_id);
