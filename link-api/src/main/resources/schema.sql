...
-- 16. Java cache replay event table
CREATE TABLE IF NOT EXISTS cache_replay_event (
    id                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    event_key           VARCHAR(384) NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_cache_replay_event_key ON cache_replay_event(event_key);
CREATE INDEX IF NOT EXISTS idx_cache_replay_status_failed ON cache_replay_event(status, last_failed_at);
